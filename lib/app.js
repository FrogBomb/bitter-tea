  ...
  if (!e.target.matches('.functionText')) {
    ...
    for (var d = 0; d < dropdowns.length; d++) {
		...
		var option_0 = document.createElement('option');
                option_0.value = "null";
                option_0.appendChild( document.createTextNode("Pick a Function!")
                );
        dd.appendChild( option_0 );
		for(var f = 0; f<funcNames.length; f++){
			var op = document.createElement('option');
                op.value = funcNames[f];
                op.appendChild( document.createTextNode(funcNames[f])
                );
             dd.appendChild( op );
		}
    }
